...
import 'package:hushh_app/app/shared/core/components/custom_qr/neighbors.dart';
import 'package:hushh_app/app/shared/core/components/custom_qr/utils.dart';
...
mixin ShapeRect implements QrElementShape {
  @override
  Path createPath(Offset offset, double size, Neighbors neighbors) =>
      Path()..addRect(Rect.fromLTWH(offset.dx, offset.dy, size, size));
}
  ...
  Path createPath(Offset offset, double size, Neighbors neighbors) {
    ...
    return Path()
      ..addOval(Rect.fromLTWH(
          offset.dx + padding, offset.dy + padding, sSize, sSize));
  }
}
...
mixin ShapeRoundCorners implements QrElementShape {
  ...
  Path createPath(Offset offset, double size, Neighbors neighbors) {
    ...
    if (!neighbors.hasAny()) {
      path.addRRect(RRect.fromLTRBR(offset.dx, offset.dy, offset.dx + size,
          offset.dy + size, Radius.circular(corner)));
    } else {
      double topLeft = 0;
      double topRight = 0;
      double bottomLeft = 0;
